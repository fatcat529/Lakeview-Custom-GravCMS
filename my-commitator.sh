...
function generate-commits() {
    ...
    for TIME_MACHINE in $(seq $START 86400 $END); do
        ...
        # if [ "$DAY" != 'Saturday' ] && [ "$DAY" != 'Sunday' ];then
            ...
            DAY_PROB=50 # probability of working on days
            SAT_SUN_PROB=50 # probability of working on Saturday and Sunday

            RND=0
            PROBABILITY=$(( $RANDOM % 100 ))
            if [ $PROBABILITY -lt $DAY_PROB ]; then
                PROBABILITY=$(( $RANDOM % 100 ))
                if [ $PROBABILITY -lt 10 ]; then
                    RND=$(( ($RANDOM % 5) + 5 ))
                elif [ $PROBABILITY -lt 25 ]; then
                    RND=4
                elif [ $PROBABILITY -lt 45 ]; then
                    RND=3
                elif [ $PROBABILITY -lt 60 ]; then
                    RND=2
                elif [ $PROBABILITY -lt 90 ]; then
                    RND=1
                else
                    RND=0
                fi
                if [ "$DAY" = 'Saturday' ] && [ "$DAY" = 'Sunday' ];then
                    PROBABILITY=$(( $RANDOM % 100 ))
                    if [ $PROBABILITY -gt $SAT_SUN_PROB ]; then
                        RND=0
                    fi
                fi
            else
